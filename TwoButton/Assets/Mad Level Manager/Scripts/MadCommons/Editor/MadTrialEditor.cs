...
public class MadTrialEditor : Editor {
    ...
    public static bool expired {
        get {
            ...
            int startDate = EditorPrefs.GetInt(StartKey);
            int now = Now();
    ...
    private static bool TrialStarted() {
        return EditorPrefs.HasKey(StartKey);
    }

    private static void StartTrial() {
        int startTime = Now();
        EditorPrefs.SetInt(StartKey, startTime);
    }
    ...
    private static bool CanNotifyTrialEnded() {
        ...
        bool result = false;
        if (EditorPrefs.GetInt(DayKey, -1) != day) {
            result = true;
        }
    ...
    public static void Extend(string key) {
        ...
        if (LoadKey(key, out newStart)) {
            EditorPrefs.SetInt(StartKey, newStart);
