...
public class GoogleCloudUseExample : MonoBehaviour {
	...
	void OnGUI() {
		if(GUI.Button(new Rect(490, 130, 150, 50), "Load All States")) {
			GoogleCloudManager.instance.loadAllStates ();
		}

		if(GUI.Button(new Rect(490, 190, 150, 50), "Load Sate")) {
			GoogleCloudManager.instance.loadState (GoogleCloudSlot.SLOT_0);
		}

		
		if(GUI.Button(new Rect(490, 250, 150, 50), "Update state")) {
		...
		if(GUI.Button(new Rect(490, 310, 150, 50), "Delete state")) {
