...
public class GooglePlayManager : Singletone<GooglePlayManager> {
	...
	public const string PLAYERS_LOADED            = "players_loaded";
	...
	private void OnScoreDataRecevide(string data) {
		...
		if(result.isSuccess) {
			...
			for(int i = 5; i < storeData.Length; i+=6) {
				if(storeData[i] == AndroidNative.DATA_EOF) {
					break;
				}
				...
				if(!players.ContainsKey(playerId)) {
					GooglePlayerTemplate p = new GooglePlayerTemplate (playerId, storeData[i + 3], storeData[i + 4], storeData[i + 5]);
					AddPlayer(p);
				}
	...
	private void OnPlayerDataLoaded(string data) {
		...
		_player = new GooglePlayerTemplate (storeData [0], storeData [1], storeData [2], storeData [3]);
		AddPlayer(_player);
	}
	...
	private void OnPlayersLoaded(string data) {
		...
		if(result.isSuccess) {
			...
			for(int i = 1; i < storeData.Length; i+=4) {
				...
				GooglePlayerTemplate p = new GooglePlayerTemplate (storeData[i], storeData[i + 1], storeData[i + 2], storeData[i + 3]);
				AddPlayer(p);
		...
		dispatch (PLAYERS_LOADED, result);
	}
