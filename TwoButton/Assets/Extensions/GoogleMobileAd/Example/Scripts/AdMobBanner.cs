...
public class AdMobBanner : MonoBehaviour {
	...
	public string AndroidBannersUnityId;
	public string IOSBannersUnityId;
	...
	void Awake() {
		...
		if(GoogleMobileAd.IsInited) {
			...
			string unit_id = AndroidBannersUnityId;
			if(Application.platform == RuntimePlatform.IPhonePlayer) {
				unit_id = IOSBannersUnityId;
			}


			if(!GoogleMobileAd.BannersUunitId.Equals(unit_id)) {
				GoogleMobileAd.SetBannersUnitID(AndroidBannersUnityId, IOSBannersUnityId);
			}


		} else {
			GoogleMobileAd.Init(AndroidBannersUnityId, IOSBannersUnityId);
		}
